...
import { View, Text, Navigator, TouchableHighlight, StyleSheet, AsyncStorage } from 'react-native';
...
export default class AttributeCard extends Component {
  ...
  componentDidMount() {
      ...
      const storedState = AsyncStorage.getItem(this.props.title, (error, result) => {
          if (result !== null) {
  ...
  saveData(data) {
      this.setState(data, () => {
        AsyncStorage.setItem(this.props.title, JSON.stringify(this.state), (error) => {
